...
const ContactButton: React.FC<ContactButtonProps> = ({ 
  ...
  if (variant === 'secondary') {
    return (
      <div className={`relative ${className}`}>
        ...
        >
          <MessageCircle size={20} className="text-blue-400 group-hover:scale-110 transition-transform duration-200" />
          ...
          <div className="absolute top-full left-0 mt-2 w-64 bg-slate-900/95 backdrop-blur-md border border-blue-500/20 rounded-lg shadow-lg p-4 z-50">
            <div className="space-y-3">
              ...
              >
                ...
                <span className="text-blue-400 text-sm">{t('footer.workTogether')}</span>
              </button>
            </div>
          </div>
        )}
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      ...
      >
        <Send className="group-hover:scale-110 transition-transform duration-200" size={18} />
